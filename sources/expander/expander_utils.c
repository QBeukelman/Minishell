/* ************************************************************************** */
/*                                                                            */
/*                                                        ::::::::            */
/*   expander_utils.c                                   :+:    :+:            */
/*                                                     +:+                    */
/*   By: qbeukelm <qbeukelm@student.42.fr>            +#+                     */
/*                                                   +#+                      */
/*   Created: 2024/04/23 21:54:16 by quentinbeuk   #+#    #+#                 */
/*   Updated: 2024/06/20 15:06:47 by qtrinh        ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/minishell.h"
#include "../../includes/libft/lib_vector/vector.h"

int	count_expand(char *arg)
{
	int		i;
	int		count;

	i = 0;
	count = 0;
	while (arg[i])
	{
		if (arg[i] == EXPAND_CHAR)
			count++;
		i++;
	}
	return (count);
}

bool is_arg_key(char *arg, char *key)
{
	if (ft_strncmp(arg, key, (ft_strlen(key) + ft_strlen(arg)))  == 0)
		return (true);
	return (false);
}

char *expand_exit_code(char *arg, size_t i, t_vec *vec)
{
	char	*exit_code_string;
	char	*leading_substr;
	char	*trailing_substr;

	exit_code_string = ft_itoa(g_exit_code);
	leading_substr = ft_substr(arg, 0, i);
	trailing_substr = ft_substr(arg, i + 2, ft_strlen(arg));
	ft_vec_push_str(vec, leading_substr);
	ft_vec_push_str(vec, exit_code_string);
	ft_vec_push_str(vec, trailing_substr);
	free_strs(exit_code_string, leading_substr, trailing_substr);
	return (free(arg), ft_vec_to_str(vec));
}
